...
u32 ic_version_a_b = FALSE;
u32 aldo_fix_need = FALSE;
...
s32 pmu_clear_pendings(void)
{
	...
	if (pmu_ops_p->pmu_clear_pendings)
		ret = pmu_ops_p->pmu_clear_pendings();
...
s32 pmu_init(void)
{
	...
	ic_version_a_b = ((readl(SYS_CFG_VER_REG) & 0x7) < 0x2) ? TRUE : FALSE;
	aldo_efuse = (readl(SID_ALDO_EFUSE) & 0x10000) ? TRUE : FALSE;

	if (ic_version_a_b && aldo_efuse) {
		aldo_fix_need = TRUE;
	}
