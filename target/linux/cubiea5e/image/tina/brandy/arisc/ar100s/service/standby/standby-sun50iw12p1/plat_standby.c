...
{
	if (read_fake_poweroff_flag() == FAKE_POWEROFF) {
		/* disable vdd-cpu */
		pin_set_multi_sel(vdd_cpu_grp, vdd_cpu_num, 0x1);
		writel(readl(PIN_REG_DATA(vdd_cpu_grp)) & (~(0x1 << vdd_cpu_num)), PIN_REG_DATA(vdd_cpu_grp));

		/* disable vdd-sys */
		pin_set_multi_sel(vdd_sys_grp, vdd_sys_num, 0x1);
		writel(readl(PIN_REG_DATA(vdd_sys_grp)) & (~(0x1 << vdd_sys_num)), PIN_REG_DATA(vdd_sys_grp));

		/* disable vcc-pll */
		pin_set_multi_sel(vcc_pll_grp, vcc_pll_num, 0x1);
		writel(readl(PIN_REG_DATA(vcc_pll_grp)) & (~(0x1 << vcc_pll_num)), PIN_REG_DATA(vcc_pll_grp));

		/* disable vcc-dram */
		pin_set_multi_sel(vcc_dram_grp, vcc_dram_num, 0x1);
		writel(readl(PIN_REG_DATA(vcc_dram_grp)) & (~(0x1 << vcc_dram_num)), PIN_REG_DATA(vcc_dram_grp));
