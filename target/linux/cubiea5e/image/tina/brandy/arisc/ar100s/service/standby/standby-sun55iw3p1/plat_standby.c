...
static s32 standby_dram_dts_parse(void)
{
	...
	if (platform_dram_para) {
		_dts_dram_para = platform_dram_para;
		dts_has_parsed = 1;
		return 0;
	}
	...
	if (fdt_getprop_u32(fdt, dram_para_node, "dram_para[00]", &dts_dram_para[0]) < 0) {
		...
		for (i = 0; i < (sizeof(dts_dram_para) / sizeof(u32)); i++) {
			if (i < 10) {
				sprintf(dram_para_prop, "dram_para[0%d]", i);
			} else {
				sprintf(dram_para_prop, "dram_para[%d]", i);
			}
			fdt_getprop_u32(fdt, dram_para_node, dram_para_prop, &dts_dram_para[i]);
...
static s32 fake_poweroff_pin_dts_parse(void)
{
	...
	/* parse box node */
	box_node = fdt_path_offset(fdt, "box-start-os0");
	if (box_node < 0)
		return -1;
...
static s32 fake_poweroff_pmu_dts_parse(void)
{
	/* do nothing now */
...
static s32 standby_dts_parse(void)
{
	...
	if (fdt_getprop_u32(fdt, param_node, "pmu-latency-ms", &pmu_latency_ms) < 0)
		pmu_latency_ms = 0;
	if (pmu_latency_ms)
		printk("set pmu-latency-ms: %d\n", pmu_latency_ms);
...
static void aldo_suspend_resume(void)
{
	u32 type;
...
{
	/* disable MCU audio pll */
	writel((readl(MCU_PLL_AUDIO_REG) & (~(PLL_ENABLE_MASK | PLL_LDO_ENABLE_MASK))), MCU_PLL_AUDIO_REG);
	writel((readl(MCU_PLL_AUDIO_REG) & (~PLL_LOCK_ENABLE_MASK)), MCU_PLL_AUDIO_REG);
}
...
static void system_suspend(void)
{
	...
	writel((readl(ANA_PWR_RST_REG) & (~ANA_EN_MCU_MASK)) | ANA_EN_MCU_CTRL(1), ANA_PWR_RST_REG);
	...
	/* SYS isolation */
	writel((readl(VDD_SYS_PWROFF_GATING_REG) & (~VDD_SYS2MCU_GATING_MASK)) | VDD_SYS2MCU_GATING(1), VDD_SYS_PWROFF_GATING_REG);
	writel((readl(VDD_SYS_PWROFF_GATING_REG) & (~VDD_SYS2CPUS_GATING_MASK)) | VDD_SYS2CPUS_GATING(1), VDD_SYS_PWROFF_GATING_REG);

	/* MCU isolation */
	if (!standby_mcu_en) {
		writel((readl(MCU_SYS_PWR_RST_REG) & (~MCU_SYS_MODULE_RST_MASK)) | MCU_SYS_MODULE_RST(0), MCU_SYS_PWR_RST_REG);
		writel((readl(VDD_SYS_PWROFF_GATING_REG) & (~VDD_MCU2CPUS_GATING_MASK)) | VDD_MCU2CPUS_GATING(1), VDD_SYS_PWROFF_GATING_REG);
	}
}
...
static void system_resume(void)
{
	/* MCU isolation */
	if (!standby_mcu_en) {
		writel((readl(MCU_SYS_PWR_RST_REG) & (~MCU_SYS_MODULE_RST_MASK)) | MCU_SYS_MODULE_RST(1), MCU_SYS_PWR_RST_REG);
		writel((readl(VDD_SYS_PWROFF_GATING_REG) & (~VDD_MCU2CPUS_GATING_MASK)) | VDD_MCU2CPUS_GATING(0), VDD_SYS_PWROFF_GATING_REG);
	}

	/* SYS isolation */
	writel((readl(VDD_SYS_PWROFF_GATING_REG) & (~VDD_SYS2MCU_GATING_MASK)) | VDD_SYS2MCU_GATING(0), VDD_SYS_PWROFF_GATING_REG);
	...
	writel((readl(ANA_PWR_RST_REG) & (~ANA_EN_MCU_MASK)) | ANA_EN_MCU_CTRL(0), ANA_PWR_RST_REG);
...
static void fake_poweroff_pins_wakeup_init(void)
{
...
static void fake_poweroff_pmu_wakeup_init(void)
{
...
s32 fake_poweroff(struct message *pmessage)
{
	...
	save_state_flag(REC_FAKEPOWEROFF | REC_BEFORE_INIT);
	standby_process_init(pmessage);
	save_state_flag(REC_FAKEPOWEROFF | REC_AFTER_INIT);

	LOG("wait wakeup\n");
	save_state_flag(REC_FAKEPOWEROFF | REC_WAIT_WAKEUP);
