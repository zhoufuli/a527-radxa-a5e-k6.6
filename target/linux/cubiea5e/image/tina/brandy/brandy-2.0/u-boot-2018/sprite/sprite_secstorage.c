...
*                                             function
*
*name          :  __probe_name_in_map Find the specified item in the secure storage map
*
*parmeters     :  buffer :
*
*    return        :
...
int sunxi_secure_storage_write(const char *item_name, char *buffer, int length)
{
	...
	if (index < 0) {
		...
		pr_force(
			"There is the same name in the secure storage, try to erase it\n");
		if (len != length) {
