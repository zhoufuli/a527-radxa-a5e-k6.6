...
static s32 hdmi_enable(void)
{
	...
	if (hdmi_drv->hdmi_clk) {
		if (hdmi_drv->tcon_tv_clk)
			...
			hdmi_err("tcon_tv clk get failed\n");
...

{
...
#else
	/* iomap */
	reg_base = disp_getprop_regbase("/soc/hdmi", "reg", 0);
	if (reg_base == 0) {
		pr_err("unable to map hdmi registers\n");
		ret = -1;
		kfree(hdmi_drv);
	...
	node_offset = disp_fdt_nodeoffset("/soc/hdmi");
	...
	if (IS_ERR(hdmi_drv->hdmi_hdcp_clk)) {
		hdmi_err("fail to get clk for hdmi hdcp\n");
		kfree(hdmi_drv);
		return -1;
	}
	count++;
...
#if defined(CONFIG_MACH_SUN55IW3)
	hdmi_drv->hdmi_24M_clk = of_clk_get(node_offset, count);
	if (hdmi_drv->hdmi_24M_clk == NULL) {
		hdmi_err("fail to get clk for hdmi 24m\n");
		kfree(hdmi_drv);
		return -1;
	} else {
		ret = clk_prepare_enable(hdmi_drv->hdmi_24M_clk);
		if (ret != 0)
			hdmi_err("hdmi 24 clk enable failed!\n");
	}
	count++;

	hdmi_drv->hdmi_sub_clk = of_clk_get(node_offset, count);
	if (hdmi_drv->hdmi_sub_clk == NULL) {
		hdmi_err("fail to get clk for hdmi sub\n");
		kfree(hdmi_drv);
		return -1;
	} else {
		ret = clk_prepare_enable(hdmi_drv->hdmi_sub_clk);
		if (ret != 0)
			hdmi_err("hdmi sub clk enable failed!\n");
	}
	count++;
#endif

#if defined(CONFIG_MACH_SUN55IW3)
	hdmi_drv->hdmi_cec_clk = NULL;
#else
	hdmi_drv->hdmi_cec_clk = of_clk_get(node_offset, count);
	if (IS_ERR(hdmi_drv->hdmi_cec_clk)) {
		hdmi_err("fail to get clk for hdmi cec\n");
		kfree(hdmi_drv);
		return -1;
	}
	count++;
	...
	if (IS_ERR(hdmi_drv->tcon_tv_clk)) {
		hdmi_err("fail to get clk for tcon tv\n");
		kfree(hdmi_drv);
		return -1;
	}
...
static struct hdmi_debug_video_mode debug_video_mode[] = {
	...
	{HDMI_VIC_640x480P60_4_3, "640x480P"},
 ...
 * Uboot Platform
*/
#if defined (CONFIG_HDMI20_UBOOT_DEBUG)
struct hdmi_mode_to_name {
	int hdmi_mode;/* vic */
	char *name;
};

static char *string_color_space[] = {
	"RGB",
	"YUV422",
	"YUV444",
	"YUV420",
...
static char *string_colori_metry[] = {
	"NULL",
	"ITU601",
	"ITU709",
	"XV_YCC601",
...
static struct hdmi_mode_to_name video_name[] = {
	{HDMI_VIC_720x480I_4_3, "480I"},
	{HDMI_VIC_720x480I_16_9, "480I"},
	{HDMI_VIC_720x576I_4_3,	"576I"},
	{HDMI_VIC_720x576I_16_9, "576I"},
	...
	/* vesa video name */
	{HDMI_VIC_640x480P60,         "VESA_640x480P60"},
	{HDMI_VIC_800x600P60,         "VESA_800x600P60"},
	{HDMI_VIC_1024x768P60,        "VESA_1024x768P60"},
	{HDMI_VIC_1280x768P60_5x4,    "VESA_1280x768P60"},
	{HDMI_VIC_1280x800P60_16x10,  "VESA_1280x800P60"},
	{HDMI_VIC_1280x960P60_4x3,    "VESA_1280x960P60"},
	{HDMI_VIC_1280x1024P60_5x4,   "VESA_1280x1024P60"},
	{HDMI_VIC_1360x768P60_16x9,   "VESA_1360x768P60"},
	{HDMI_VIC_1366x768P60_16x9,   "VESA_1366x768P60"},
	{HDMI_VIC_1400x1050P60_4x3,   "VESA_1400x1050P60"},
	{HDMI_VIC_1440x900P60_16x10,  "VESA_1440x900P60"},
	{HDMI_VIC_1600x900P60_16x9,   "VESA_1600x900P60"},
	{HDMI_VIC_1680x1050P60_16x10, "VESA_1690x1050P60"},
	{HDMI_VIC_1920x1200P60_16x10, "VESA_1920x1200P60"},
	{HDMI_VIC_1920x1440P60_4x3,   "VESA_1920x1440P60"},
};

char *string_video_name(int hdmi_mode)
{
	int i = 0;

	for (i = 0;
	i < sizeof(video_name)/sizeof(struct hdmi_mode_to_name);
	 i++) {
		if (video_name[i].hdmi_mode == hdmi_mode)
			return video_name[i].name;
	}

	return NULL;
}

static ssize_t _aw_hdmi_source_show(char *buf, size_t n)
{
	n += sprintf(buf+n, "\n%s%d\n",
		"HPD:  ",
		hdmi_core_get_hpd_state());
	...
	n += sprintf(buf+n, "\n%s%s\n",
		"color space:  ",
		string_color_space[hdmi_core_get_pixel_format()]);
	n += sprintf(buf+n, "\n%s%s\n",
		"Colorimetry:  ",
		 string_colori_metry[hdmi_core_get_colorimetry()]);
	n += sprintf(buf+n, "\n%s%s\n",
		"VideoFormat:  ",
		 string_video_name((int)hdmi_core_get_video_code()));
	n += sprintf(buf+n, "\n%s%d\n",
		"AudioLayout:  ",
		 hdmi_core_get_audio_layout());
	n += sprintf(buf+n, "\n%s%d\n",
...
static int _uboot_dump_hdmi_source(void)
{
	#define HDMI_SOURCE_BUF_SIZE 1024
	char *buff = NULL;
	size_t count = 0;

	buff = kmalloc(HDMI_SOURCE_BUF_SIZE, GFP_KERNEL | __GFP_ZERO);
	if (buff == NULL) {
		hdmi_err("kmalloc buffer failed!!!\n");
		return -1;
	}

	if (_aw_hdmi_source_show(buff, count) > HDMI_SOURCE_BUF_SIZE)
		hdmi_wrn("dump buffer is too small!\n");
...
int do_sunxi_hdmi20_show(cmd_tbl_t *cmdtp,
	int flag, int argc, char *const argv[])
{
	...
	snprintf(cmd_name, 32, "%s", argv[1]);

	if (strcmp("hdmi_source", cmd_name) == 0) {
		return _uboot_dump_hdmi_source();
#if defined(CONFIG_HDMI20_LOG_BUFFER)
	} else if (strcmp("hdmi_log", cmd_name) == 0) {
		return _uboot_dump_hdmi_log();
#endif
	} else {
		return cmd_usage(cmdtp);
	}
...
U_BOOT_CMD(
	sunxi_hdmi20, 2, 0, do_sunxi_hdmi20_show,
	"sunxi hdmi20 debug cmd",
	"\nparameters : hdmi_source\n"
		"\t - dump hdmi source info"
#if defined(CONFIG_HDMI20_LOG_BUFFER)
	"\nparameters : hdmi_log\n"
		"\t - dump uboot hdmi log"
#endif
);
#endif /* CONFIG_HDMI20_UBOOT_DEBUG */
