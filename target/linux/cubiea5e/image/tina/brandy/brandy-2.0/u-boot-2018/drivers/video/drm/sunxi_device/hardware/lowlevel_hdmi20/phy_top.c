...
#define PHY_DCXO_24M	(0x0)
#define PHY_DCXO_19_2M	(0x1)
#define PHY_DCXO_26M	(0x2)
...
int top_phy_config(void)
{
	...
	/* check min clock */
	if (raw_clk < pll_cfg[0].tmds_clock) {
		index = 0;
		goto cfg_pll;
	}

	if (raw_clk > pll_cfg[size - 1].tmds_clock) {
		index = size - 1;
		goto cfg_pll;
	}

	for (i = 0; i < size; i++) {
		if (raw_clk == pll_cfg[i].tmds_clock) {
			index = i;
			goto cfg_pll;
		}

		if (raw_clk > pll_cfg[i].tmds_clock &&
				raw_clk < pll_cfg[i + 1].tmds_clock) {
			if ((pll_cfg[i + 1].tmds_clock - raw_clk) >
					(raw_clk - pll_cfg[i].tmds_clock))
				index = i;
			else
				index = i + 1;
			goto cfg_pll;
