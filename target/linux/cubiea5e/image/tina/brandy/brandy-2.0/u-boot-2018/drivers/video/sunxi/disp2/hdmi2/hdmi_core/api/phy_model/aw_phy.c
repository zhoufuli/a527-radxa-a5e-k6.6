...
static u32 _aw_phy_get_freq(u32 pClk)
{
	if (((pClk >= 25175) && (pClk <= 25180)) ||
		...
		pr_err("Unable to map input pixel clock frequency %d kHz\n", pClk);
	}
	return 1000;
}

static struct aw_phy_config *_aw_phy_get_configs(u32 pClk, color_depth_t color,
...
static void _aw_phy_encoding_adapt(encoding_t EncodingOut)
{
...
static int _aw_phy_configure(hdmi_tx_dev_t *dev, u32 pClk, color_depth_t color,
						pixel_repetition_t pixel,
						encoding_t EncodingOut)
{
	...
	if (config == NULL) {
		pr_err("failed to get phy config when clock %dhz, color depth %d, pixel repetition %d\n",
			pClk, color, pixel);
		return -1;
	}
...
int aw_phy_configure(hdmi_tx_dev_t *dev, encoding_t EncodingOut)
{
	LOG_TRACE();
