...
typedef struct hal_fb_dev {
	...
	disp_device_t *disp_dev[DISP_DEV_NUM];
} hal_fb_dev_t;
...
int hal_switch_device(disp_device_t *device, unsigned int fb_id)
{
	...
	fb_dev->disp_dev[fb_dev->dev_num] = device;
...
int hal_get_panel_convert_is_enabeld(void)
{
	return _get_panel_convert_is_enabled();
}
...
int hal_save_boot_disp(void *handle)
{
	...
	char name[12];
	char disp_paras[128];
	static int boot_disp;

	if (!(fb_dev->state & DISP_DEV_OPENED))
		return -1;

	for (i = 0; i < fb_dev->dev_num; i++) {
		memset(name, 0, sizeof(name));
		memset(disp_paras, 0, sizeof(disp_paras));

		snprintf(name, sizeof(name), "boot_disp%d", fb_dev->disp_dev[i]->screen_id);
		snprintf(disp_paras, sizeof(disp_paras), "%x,%x,%x,%x,%x,%x",
				fb_dev->disp_dev[i]->type, fb_dev->disp_dev[i]->mode,
				fb_dev->disp_dev[i]->format, fb_dev->disp_dev[i]->bits,
				fb_dev->disp_dev[i]->cs, fb_dev->disp_dev[i]->eotf);
		hal_save_string_to_kernel(name, disp_paras);

		/* TODO: Remove boot_disp */
		if (fb_dev->disp_dev[i]->screen_id < 2) {
			boot_disp &= ~(0xFFFF << (fb_dev->disp_dev[i]->screen_id * 16));
			boot_disp |= (((fb_dev->disp_dev[i]->type << 8) | fb_dev->disp_dev[i]->mode)
					<< (fb_dev->disp_dev[i]->screen_id * 16));
		}
	}

	hal_save_int_to_kernel("boot_disp", boot_disp);
