...
#define DE_SETMASK(width, shift)   (((width)?((-1U) >> (32-(width))):0)  << (shift))
#define DE_CLRMASK(width, shift)   (~(DE_SETMASK(width, shift)))
#define DE_GET_BITS(shift, width, reg)     \
	(((reg) & DE_SETMASK(width, shift)) >> (shift))
#define DE_SET_BITS(shift, width, reg, val) \
	(((reg) & DE_CLRMASK(width, shift)) | ((val) << (shift)))
