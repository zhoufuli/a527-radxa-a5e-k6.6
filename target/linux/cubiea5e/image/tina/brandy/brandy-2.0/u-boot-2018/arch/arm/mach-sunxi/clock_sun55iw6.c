...
int clock_set_corepll(int frequency)
{
	...
	struct sunxi_cpu_pll_reg *const ccm =
		(struct sunxi_cpu_pll_reg *)CCMU_PLL_CPU_CFG_REG;
	...
	/* switch to 24M*/
	reg_val = 0x0305;
	writel(reg_val, &ccm->cpu_clk_reg);
	__udelay(20);
	...
	enable_pll((u32)&ccm->cpu_l0_pll_ctrl_reg, &pll_factor);

	pll_factor.FactorP = 0;
	pll_factor.FactorM = 0;
	pll_factor.FactorN = 0x27; /*936M*/
	enable_pll((u32)&ccm->cpu_dsu_pll_ctrl_reg, &pll_factor);

	/* switch core clk src to PLL_CPU1  PLL_CPU1/P*/
	reg_val = readl(&ccm->cpu_clk_reg);
	reg_val &= ~(0x03 << 24);
	reg_val |=  (0x03 << 24);
	reg_val &= ~(0x01 << 16); // P = 1
	reg_val |= (0x00 << 16);
...
uint clock_get_corepll(void)
{
	...
	case 2://RC16M
		...
		break;
	case 3://PLL_CPU1
		reg_val  = readl(&ccm->cpu_l0_pll_ctrl_reg);
...
uint clock_get_mbus(void)
{
	...
	printf("mbus value clk value:0x%x\n", reg_val);
	//get src
	src = (reg_val >> 24) & 0x3;
