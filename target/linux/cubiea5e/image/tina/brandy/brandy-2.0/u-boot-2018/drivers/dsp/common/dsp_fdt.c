...
int dts_get_dsp_memory(ulong *start, u32 *size, u32 id)
{
	...
	if (id  == 0) {
		nodeoffset = fdt_path_offset(working_fdt, "/reserved-memory/dsp0ddr");
		if (nodeoffset < 0) {
			pr_err("%s: no /reserved-memory/dsp0 in fdt\n", __func__);
			return -1;
		}
	} else {
		nodeoffset = fdt_path_offset(working_fdt, "/reserved-memory/dsp1ddr");
		if (nodeoffset < 0) {
	...
	memset(reg_data, 0, sizeof(reg_data));
	ret = fdt_getprop_u32(working_fdt, nodeoffset, "reg", reg_data);
	if (ret < 0) {
