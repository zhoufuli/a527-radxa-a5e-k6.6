	...
	unsigned int *value, unsigned int defval)
{
	if (fdt_getprop_u32(working_fdt, node, name, value) < 0) {
		pr_msg("get disp.%s fail. using defval=%u\n", name, defval);

		*value = defval;
	}
}

static int fdt_iova_add(int node_offset, unsigned long address, unsigned int length)
{
	int ret;
...
int hal_add_kernel_iova_premap(unsigned long address, unsigned int length)
{
	int ret;
	int disp_node = get_disp_fdt_node();
	int drm_de_node = fdt_path_offset(working_fdt, "/soc/de");

	ret = fdt_iova_add(disp_node, address, length);
	if (ret)
		return ret;

	if (drm_de_node > 0) {
		ret = fdt_iova_add(drm_de_node, address, length);
	}
	return ret;
}
...
{
	int ret = -1;
	int drm_de_node = fdt_path_offset(working_fdt, "/soc/sunxi-drm");

#ifndef CONFIG_SUNXI_MULITCORE_BOOT
	int node = get_disp_fdt_node();
	ret = fdt_setprop_string(working_fdt, node, name, str);
	if (drm_de_node > 0)
		ret = fdt_setprop_string(working_fdt, drm_de_node, name, str);
#else
...
int hal_fat_fsload(char *part_name, char *file_name, char *buf, ulong length)
{
	...
	if (partno < 0) {
		pr_msg("Get %s partition number fail!\n", part_name);
		return -1;
	}
