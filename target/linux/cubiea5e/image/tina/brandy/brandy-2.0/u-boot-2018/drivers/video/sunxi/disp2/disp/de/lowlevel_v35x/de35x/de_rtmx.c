...
void de_get_amp_preview_info(u32 disp, u32 chn, struct disp_amp_preview_channel_info *p)
{
	...
	struct de_rtmx_context *ctx = de_rtmx_get_context(disp);
	asu_support = de_feat_is_support_asuscale_by_chn(disp, chn);

	if (asu_support) {
		p->alpha_reg = (unsigned long)(ctx->de_reg_base + DE_CHN_OFFSET(phy_chn)
				+ CHN_SCALER_OFFSET + 0x4);
		p->alpha_shift = 0;
	} else {
		p->alpha_reg = (unsigned long)(ctx->de_reg_base + DE_CHN_OFFSET(phy_chn)
				+ CHN_OVL_OFFSET);
	...
	p->addr_reg = (unsigned long)(ctx->de_reg_base + DE_CHN_OFFSET(phy_chn)
			+ CHN_OVL_OFFSET + (chn < vcount ? 0x18 : 0x10));
	p->fill_color_en_reg = (unsigned long)(ctx->de_reg_base + DE_CHN_OFFSET(phy_chn) + CHN_OVL_OFFSET);
