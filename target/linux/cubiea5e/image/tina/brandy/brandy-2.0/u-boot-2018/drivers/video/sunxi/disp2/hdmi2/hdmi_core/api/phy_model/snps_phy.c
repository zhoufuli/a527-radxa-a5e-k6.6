...
static struct phy_config301 *_snps_phy_301_get_configs(u32 pClk, color_depth_t color,
...
static int _snps_phy301_configure(hdmi_tx_dev_t *dev, u32 pClk, color_depth_t color,
						pixel_repetition_t pixel)
{
	u16 phyRead = 0;
	...
	if (snps_phy_read(dev, OPMODE_PLLCFG, &phyRead) || (phyRead != config->oppllcfg))
		pr_err("OPMODE_PLLCFG Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (snps_phy_read(dev, PLLCURRCTRL, &phyRead) || (phyRead != config->pllcurrctrl))
		pr_err("PLLCURRCTRL Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (phyRead != 3)
		pr_err("PLLGMPCTRL Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (snps_phy_read(dev, TXTERM, &phyRead) || (phyRead != config->txterm))
		pr_err("TXTERM Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (snps_phy_read(dev, CKSYMTXCTRL, &phyRead) || (phyRead != config->cksymtxctrl))
		pr_err("CKSYMTXCTRL Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (snps_phy_read(dev, VLEVCTRL, &phyRead) || (phyRead != config->vlevctrl))
		pr_err("VLEVCTRL Mismatch Write 0x%04x Read 0x%04x\n",
	...
	if (dev->snps_hdmi_ctrl.pixel_clock == 594000) {
		...
		if (snps_phy_read(dev, 0x17, &phyRead) || (phyRead != 0x8006))
			pr_err("VLEVCTRL Mismatch Write 0x8006 Read 0x%04x\n",
							phyRead);
	}
	...
	if (ctrl_phy_wait_lock(dev) == 1) {
		pr_info("phy 301 pll locked!\n");
		return TRUE;
	}

	pr_err("phy 301 pll not locked!!!\n");
	return FALSE;
}
...
u32 _snps_phy_303_get_freq(u32 pClk)
{
	if (((pClk >= 25175) && (pClk <= 25180)) ||
		...
		pr_err("Unable to map input pixel clock frequency %dkHz\n", pClk);
	}
	return 1000;
}

static struct phy_config303 *_snps_phy_303_get_configs(u32 pClk, color_depth_t color,
		pixel_repetition_t pixel)
{
...
static int _snps_phy303_configure(hdmi_tx_dev_t *dev, u32 pClk, color_depth_t color,
						pixel_repetition_t pixel)
{
	int i   = 0;
	u16 phyRead = 0;
	...
	if (ctrl_phy_wait_lock(dev) == 1) {
		pr_info("phy 303 pll locked!\n");
		return TRUE;
	}

	pr_err("phy 303 pll not locked!!!\n");
	return FALSE;
}
#endif
...
{
	return ctrl_phy_write(dev, addr, data);
}

int snps_phy_read(hdmi_tx_dev_t *dev, u8 addr, u16 *value)
{
	return ctrl_phy_read(dev, addr, value);
}
