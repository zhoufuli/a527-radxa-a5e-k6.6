...
s32 de_csc2_apply(struct de_csc_handle *hdl,
...
static inline struct csc_reg *de35x_get_csc_reg(struct de_csc_private *priv)
{

	return (struct csc_reg *)(priv->reg_blks[0].vir_addr);
}
...
static const int __maybe_unused table_sin[91] = {
	...
	const struct de_csc_info *in_info, const struct de_csc_info *out_info, bool en)
{
	...
	struct de_csc_private *priv = hdl->private;
	struct csc_debug_info *debug = &priv->debug;
	struct csc2_reg *regs = (struct csc2_reg *)de35x_get_csc_reg(priv);
	memset(debug, 0, sizeof(*debug));
...
int de_dcsc_apply(struct de_csc_handle *hdl, const struct de_csc_info *in_info, const struct de_csc_info *out_info, const struct bcsh_info *bcsh, int *csc_coeff)
{
