...
static void bus_clock_source(enum BUS_CLOCK_SOURCE mode)
{
	...
	bus_restore[3].busAddr = (void *)CCMU_APB0_CFG_REG;
	bus_restore[4].busAddr = (void *)CCMU_APB1_CFG_REG;
...
static u32 pclk_save;
static void fanout_prepare(void)
{
	u32 reg_val;

	/*sel rtc_32k from hosc 24M */
	reg_val = 0x1 << 24;
	writel(reg_val, CCMU_APB0_CFG_REG);

	/*enable pclk */
	pclk_save = readl(CCMU_PCLK_FAN_REG);
	reg_val = 0x1 << 31;
	writel(reg_val, CCMU_PCLK_FAN_REG);

	/*fanout1 sel pclk */
	reg_val = readl(CCMU_FAN_REG);
	reg_val &= ~(0x7 << 3);
	reg_val |= (0x6 << 3);
	writel(reg_val, CCMU_FAN_REG);
}

static void fanout_restore(void)
{
	u32 reg_val;

	/*sel hosc 24M from rtc_32k */
	reg_val = readl(CCMU_APB0_CFG_REG);
	reg_val = 0x0 << 24;
	writel(reg_val, CCMU_APB0_CFG_REG);
...
void system_suspend(sram_head_t *para)
{
	...
	fanout_prepare(); /* fanout rtc32k tfor wifi*/
...
//	udelay(20);
	...
	mdelay(2);
	fanout_restore();
	/* kernel resume the bus clk source */
