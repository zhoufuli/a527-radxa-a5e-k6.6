...
#ifdef CFG_SUNXI_CHECK_EN
int check_hardware_info(void);
#endif
...
uint32_t get_hardware_info(void)
{
	uint32_t val = 0;
	val = readl((unsigned int *)(SUNXI_SID_BASE + 0x200)) & 0xffff;
	return val;
}

uint32_t get_nv2_info(void)
{
	uint32_t val = 0;

	val = readl((unsigned int *)(SUNXI_SID_BASE + SID_SDK_VER_OFFSET));
	return val;
}
