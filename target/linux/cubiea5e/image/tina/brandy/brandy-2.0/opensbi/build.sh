...
prepare_toolchain()
{
	...
	echo "CROSS_COMPILE=${TOOLCHAIN_DIR}${RISCV_TOOLCHAIN_DIR}${RISCV_TOOLCHAIN_PREFIX}"
...
if [ "x${VENDOR}" != "x" ] && [ "x${VENDOR_IP}" != "x" ];then
	...
	if [ "x${OPENSBI_TARGET_DIR}" != "xNULL" ];then
		cp -vf ${OPENSBI_FIRMWARE_BIN_PATH}/${OPENSBI_FIRMWARE_BIN}  ${OPENSBI_TARGET_DIR}/opensbi_${BUILD_CHIP}.bin
	else
