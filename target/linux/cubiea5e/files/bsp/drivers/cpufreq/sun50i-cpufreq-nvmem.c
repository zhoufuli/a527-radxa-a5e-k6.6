...
static int sun50i_cpufreq_nvmem_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (strlen(ver_data.name)) {
       			int token = dev_pm_opp_set_prop_name(cpu_dev, ver_data.name);
        		if (token < 0) {
            			ret = token;
            			sunxi_err(NULL, "Failed to set prop name: %d\n", ret);
            			goto free_opp;
        		}
        		opp_tables[cpu] = token;
		}

		if (ver_data.version) {
			int token = dev_pm_opp_set_supported_hw(cpu_dev, &ver_data.version, 1);
    			if (token < 0) {
        			ret = token;
        			sunxi_err(NULL, "Failed to set supported HW: %d\n", ret);
        			goto free_opp;
    			}
    			opp_tables[cpu] = token;
		}
...
static int sun50i_cpufreq_nvmem_remove(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
        	if (opp_tables[cpu] < 0)  // 检查无效 token
        	    	continue;
