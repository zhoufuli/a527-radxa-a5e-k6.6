...
static ssize_t led_show(const struct class *class,
...
static ssize_t led_store(const struct class *class, const struct class_attribute *attr,
			const char *buf, size_t len)
{
	...
	if (len < 3 || len > led->led_count * 3) {
		LED_ERR("unexpected ledc len:%lu\n", len);
		return -EINVAL;
	}
...
static struct class_attribute led_class_attrs[] = {
	__ATTR(light, 0644, led_show, led_store),
	/* __ATTR_NULL, */
};
