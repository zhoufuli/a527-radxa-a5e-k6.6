	...
	if (req->iv_length) {
		if (flag == DMA_TO_DEVICE) {
			...
			req->iv_phy = dma_map_single(ce_cdev->pdevice, req->iv_buf,
									req->iv_length, (enum dma_data_direction)DMA_TO_DEVICE);
			SS_DBG("iv = %px, iv_phy_addr = 0x%lx\n", req->iv_buf, req->iv_phy);
		} else if (flag == DMA_FROM_DEVICE) {
			dma_unmap_single(ce_cdev->pdevice,
				req->iv_phy, req->iv_length, (enum dma_data_direction)DMA_FROM_DEVICE);
							...
							u32 src_length, u8 *dst_buffer)
{
	...
	if (req->key_length) {
		key_phy = dma_map_single(ce_cdev->pdevice,
					req->key_buffer, req->key_length, (enum dma_data_direction)DMA_TO_DEVICE);
	...
	if (req->ion_flag && (!padding_flag)) {
		...
		src_phy = dma_map_single(ce_cdev->pdevice, src_buffer, src_length, (enum dma_data_direction)DMA_TO_DEVICE);
	}
	...
	if (req->ion_flag) {
		...
		dst_phy = dma_map_single(ce_cdev->pdevice, dst_buffer, src_length, (enum dma_data_direction)DMA_TO_DEVICE);
	}
...
out:
	...
	if (req->key_length) {
		dma_unmap_single(ce_cdev->pdevice,
			key_phy, req->key_length, (enum dma_data_direction)DMA_FROM_DEVICE);
	}
	...
	if (req->ion_flag && (!padding_flag)) {
		...
		dma_unmap_single(ce_cdev->pdevice,
				src_phy, src_length, (enum dma_data_direction)DMA_FROM_DEVICE);
	}
	...
	if (req->ion_flag) {
		...
		dma_unmap_single(ce_cdev->pdevice,
				dst_phy, src_length, (enum dma_data_direction)DMA_FROM_DEVICE);
	}
...
static int rsa_crypto_start(crypto_rsa_req_ctx_t *req)
{
	...
	dma_unmap_single(ce_cdev->pdevice, pkey_phy, req->pkey_length,
			 (enum dma_data_direction)DMA_DEV_TO_MEM);
	/* data */
...
int do_ecc_crypto(crypto_ecc_req_ctx_t *req)
{
	...
	key_phy = dma_map_single(ce_cdev->pdevice, req->key_buffer,
				  req->key_length, (enum dma_data_direction)DMA_MEM_TO_DEV);
	SS_DBG("pkey = 0x%px, key_phy_addr = 0x%px\n", req->key_buffer,
	       (void *)key_phy);

	iv_phy = dma_map_single(ce_cdev->pdevice, req->iv_buffer,
				  req->iv_length, (enum dma_data_direction)DMA_MEM_TO_DEV);
	...
	src_phy = dma_map_single(ce_cdev->pdevice, req->src_buffer,
				  req->src_length, (enum dma_data_direction)DMA_MEM_TO_DEV);
...
int hash_crypto_start(crypto_hash_req_ctx_t *req)
{
	...
	if (req->ion_flag) {
		...
		text_phy = dma_map_single(ce_cdev->pdevice, req->text_buffer,
					  req->text_length, (enum dma_data_direction)DMA_MEM_TO_DEV);
	}
	SS_DBG("src = 0x%px, src_phy_addr = 0x%px\n", req->text_buffer,
	       (void *)text_phy);
