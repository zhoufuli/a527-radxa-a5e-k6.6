...
int sunxi_gnss_init(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(data->gpio_gnss_regon)) {
		...
		data->gpio_gnss_regon_assert =  gpiod_is_active_low(gpio_to_desc(data->gpio_gnss_regon)) ? 0 : 1;
	...
	if (!gpio_is_valid(data->gpio_gnss_rst)) {
		...
		data->gpio_gnss_rst_assert =  gpiod_is_active_low(gpio_to_desc(data->gpio_gnss_rst)) ? 0 : 1;
