...
static int rfkill_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(data->gpio_chip_en)) {
		...
		data->gpio_chip_en_assert =  gpiod_is_active_low(gpio_to_desc(data->gpio_chip_en)) ? 0 : 1;
	...
	if (!gpio_is_valid(data->gpio_power_en)) {
		...
		data->gpio_power_en_assert =  gpiod_is_active_low(gpio_to_desc(data->gpio_power_en)) ? 0 : 1;
