...
int sunxi_wlan_init(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(data->gpio_wlan_regon)) {
		...
		data->gpio_wlan_regon_assert = gpiod_is_active_low(gpio_to_desc(data->gpio_wlan_regon)) ? 0 : 1;
	...
	if (!gpio_is_valid(data->gpio_wlan_hostwake)) {
		...
		data->gpio_wlan_hostwake_assert =  gpiod_is_active_low(gpio_to_desc(data->gpio_wlan_hostwake)) ? 0 : 1;
