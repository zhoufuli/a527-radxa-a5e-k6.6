...
#define NAND_DRV_DATE 0x20230421
#define NAND_DRV_TIME 0x1633

/**
 * nand common1 version rule vx.ab data time
 * x >= 1; 00 <= ab <= 99
 */
#define NAND_COMMON1_PHY_DRV_VERSION "v2.10 2022-11-07 11:23"
...
*Note         :
*****************************************************************************/
DEFINE_SEMAPHORE(nand_physic_mutex,1);
...
const char *nand_fdt_get_prop_string_by_path(struct device_node *node, const char *path, const char *prop_name)
{
	...
	if (!node) {
		if (!strncmp(prop_name, "compatible", 10))
			np = of_find_node_by_type(NULL, "nand0");
		else
