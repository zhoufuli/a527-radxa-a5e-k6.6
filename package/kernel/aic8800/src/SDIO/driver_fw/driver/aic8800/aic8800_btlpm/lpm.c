...
static int bluesleep_probe(struct platform_device *pdev)
{
	...
	bsi->host_wake_assert =  gpiod_is_active_low(gpio_to_desc(bsi->host_wake)) ? 0 : 1;
	...
	bsi->ext_wake_assert =  gpiod_is_active_low(gpio_to_desc(bsi->ext_wake)) ? 0 : 1;
...
err3:
	...
	devm_gpiod_put(dev, gpio_to_desc(bsi->ext_wake));
err2:
	device_init_wakeup(dev, false);
err1:
	//devm_gpio_free(dev, bsi->host_wake);
	devm_gpiod_put(dev, gpio_to_desc(bsi->host_wake));
err0:
	devm_kfree(dev, bsi);
